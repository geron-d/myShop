    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (Objects.nonNull(req.getSession()) && (user.getId() > 0)) {
            ...
            if (user.getAccessLevel().equals(AccessLevel.USER)) {
                final RequestDispatcher requestDispatcher = req.getRequestDispatcher(Paths.START_PAGE_USER_PATH);
                requestDispatcher.forward(req, resp);
            } else {
                final RequestDispatcher requestDispatcher = req.getRequestDispatcher(Paths.START_PAGE_ADMIN_PATH);
                requestDispatcher.forward(req, resp);
            }
